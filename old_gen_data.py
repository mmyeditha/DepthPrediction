import json, sys, csv, cv2, os, shutil
import numpy as np
...
def remap(rgb_img, truth_heatmap):
    ...
    args:
        rgb_img: path to the rbg image
        truth_heatmap: path to the truth heatmap
    """
...
def gen_ply_files():
    """
    Given the rgb heatmap, generates ply file of point cloud
    """
    os.chdir('votenet/sunrgbd/sunrgbd_plane')
    pool = Pool

    def gen_ply_file(image_id):
        rgb_img = f'{image_id+1}/rgb_{image_id+1}.jpg'
        depth_img = f'{image_id+1}/depth_{image_id+1}.png'
        r = remap(rgb_img, depth_img)
        pc = gen_point_cloud(r, )
