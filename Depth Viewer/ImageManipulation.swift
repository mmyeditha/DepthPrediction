...
func saveSceneDepth(depthMapBuffer: CVPixelBuffer) -> PointCloud {
    ...
    let floatBuffer = unsafeBitCast(CVPixelBufferGetBaseAddress(depthMapBuffer), to: UnsafeMutablePointer<Float32>.self)
    var floatCopy = [Float32](repeating: 0.0, count: width*height)
    for i in 0..<width*height {
        floatCopy[i] = floatBuffer[i]
    }
    return PointCloud(width: width, height: height, depthData: floatCopy)
}
